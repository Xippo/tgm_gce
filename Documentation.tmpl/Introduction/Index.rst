.. ==================================================
.. FOR YOUR INFORMATION
.. --------------------------------------------------
.. -*- coding: utf-8 -*- with BOM.

.. include:: ../Includes.txt


.. _introduction:

Introduction
============


.. _what-it-does:

What does it do?
----------------

- This extension give you the possibility to create simple events(title, description, image and contact data ) and show this events as list or on a google map.
- The events support features like frequency, thanks the calendarize extension created by Loch Müller (great work!)
- You can add quite easily a fluid form to your event. Its fluid! So you can customize it at pleasure.
- The E-Mail templates (receiver,costumer), for the event forms, are fluid as well and are fully customizable.
- The Google map that shows the events can be used standalone.
- You can add some costum group (like a tag) to your event and than filter by that in the plugin. E.g. you have one group of events for London and one for Manchester. In that case you can use two plugins and show the events distinctly.
- The Plugins can filter the events by TYPO3 categories

.. _screenshots:

Screenshots
-----------
Some screeeeenshots :)

.. figure:: ../Images/TgM_GCE_intro.png
   :width: 500px
   :alt: List view with Map

   The list view with map

.. figure:: ../Images/Event_GCE_Detail.png
   :width: 500px
   :alt: Detail view with Map

   The detail view

.. figure:: ../Images/Event_GCE_with_form.png
   :width: 500px
   :alt: Detail view with from

   The detail view with form

